...
export function buildRBContext(rbWeeks: any[]): PlayerContext | { error: string } {
  if (!rbWeeks || rbWeeks.length === 0) {
    return { error: "No RB data available" };
  }

  const filtered = filterByWeeks(rbWeeks, 16);

  if (filtered.length === 0) {
    return { error: "No RB data available within weeks 1-16" };
  }
  ...
  // SEASON STATS
  // =====================
  const rushingYards = sum(sorted, "rushingYards");
  const rushingTD = sum(sorted, "rushingTD");
  ...
  const totalTD = rushingTD + receivingTD;
  ...
  // =====================
  const last3RushingYards = sum(last3, "rushingYards");
  const last3RushingTD = sum(last3, "rushingTD");
  ...
  const last3Summary = {
    ...
    avgRushingYards: Number((last3RushingYards / last3Games).toFixed(1)),
    avgRushingTD: Number((last3RushingTD / last3Games).toFixed(2)),
